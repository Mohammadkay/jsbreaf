...
btn.addEventListener('click', function (event) {
    let user = new Users('', '', '', name.value, date.value, phone.value, degree.value, '', '')
    // validation form 
    let validName = false 
    let validDate = false 
    let validPhone = false 
    let validDegree = false
        if (name.value != '' && date.value != '' && phone.value != '' && degree.value != '') {
        validName = true
        validDate = true
        validPhone = true
        validDegree = true
            ...
            if(phone.value.length < 10){
                valid_phone.textContent  = "phone number must be 10 degits"
            ...
            if(phone.value.length ==10 && name.value.length > 2 && date.value.length == 10 && degree.value > 0){
                localStorage.setItem('user', JSON.stringify(user))
                console.log(JSON.parse(localStorage.getItem('user')))
            }
        }
    
        if (validName == false || validDate == false || validPhone == false || validDegree == false) {
